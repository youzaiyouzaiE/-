    ...
    if (shareButtonImagesNameArray) {
        if (shareButtonImagesNameArray.count > 0) {
            ...
            for (int i = 1; i < shareButtonImagesNameArray.count+1; i++) {
                ...
                //有Title的时候
                CGFloat border = (Screen_Width - 3 *SHAREBUTTON_WIDTH)/4;
                if (self.isHadTitle == YES) {
                    [shareButton setFrame:CGRectMake(border+((line-1)*(border+SHAREBUTTON_WIDTH)), self.LXActivityHeight+((column-1)*(SHAREBUTTON_INTERVAL_HEIGHT+SHAREBUTTON_HEIGHT)), SHAREBUTTON_WIDTH, SHAREBUTTON_HEIGHT)];
                } else {
                    [shareButton setFrame:CGRectMake(border+((line-1)*(border+SHAREBUTTON_WIDTH)), SHAREBUTTON_INTERVAL_HEIGHT+((column-1)*(SHAREBUTTON_INTERVAL_HEIGHT+SHAREBUTTON_HEIGHT)), SHAREBUTTON_WIDTH, SHAREBUTTON_HEIGHT)];
                }
    ...
    if (shareButtonTitlesArray) {
        if (shareButtonTitlesArray.count > 0 && shareButtonImagesNameArray.count > 0) {
            for (int j = 1; j < shareButtonTitlesArray.count+1; j++) {
                ...
                CGFloat border = (Screen_Width - 3 *SHAREBUTTON_WIDTH)/4;
                if (self.isHadTitle == YES) {
                    [shareLabel setFrame:CGRectMake(border+((line-1)*(border+SHARETITLE_WIDTH)), self.LXActivityHeight+SHAREBUTTON_HEIGHT+((column-1)*(SHARETITLE_INTERVAL_HEIGHT)), SHARETITLE_WIDTH, SHARETITLE_HEIGHT)];
                }
...
{
    CGFloat margin = (Screen_Width - BUTTON_WIDTH)/2;
    UIButton *cancelButton = [[UIButton alloc] initWithFrame:CGRectMake(margin, BUTTON_INTERVAL_HEIGHT, BUTTON_WIDTH, BUTTON_HEIGHT)];
...
{
    CGFloat margin = (Screen_Width - TITLE_WIDTH)/2;
    UILabel *titlelabel = [[UILabel alloc] initWithFrame:CGRectMake(margin, TITLE_INTERVAL_HEIGHT, TITLE_WIDTH, TITLE_HEIGHT)];
