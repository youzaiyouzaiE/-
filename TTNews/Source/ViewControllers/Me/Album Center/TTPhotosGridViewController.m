...
#import "TTPhotoGridCell.h"
#import "TTPhotoTableCell.h"
...
@interface TTPhotosGridViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,UITableViewDelegate,UITableViewDataSource,PHPhotoLibraryChangeObserver,MWPhotoBrowserDelegate> {
...
- (void)iClouldImageLoadComplete:(NSNotification *)notification {
    ...
    NSLog(@"%ld",photo.index);
    TTPhotoGridCell *cell = (TTPhotoGridCell *)[_collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForRow:photo.index inSection:0]];
...
- (void)addCollectionView
{
    ...
    [_collectionView registerClass:[TTPhotoGridCell class] forCellWithReuseIdentifier:reuseIdentifier];
...
- (void)addBottomViewAndSubVeiw
{
    ...
    bottomView.backgroundColor = COLOR_NORMAL;
...
//        MWPhoto *photo = [MWPhoto photoWithAsset:asset];
...
{
    TTPhotoGridCell *cell = (TTPhotoGridCell *)[_collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForRow:index inSection:0]];
    if (_arraySelectdAssets.count + _alreadyHaveNum == _maxSelected && selected) {
        [TTProgressHUD showMsg:[NSString stringWithFormat:@"最多选择%ld张图片",(long)_maxSelected]];
        cell.selectButton.selected = NO;
        return ;
    }
    if (cell.isICloudAsset && selected) {
        [TTProgressHUD showMsg:@"此图片在iColud上,点击图片获取"];
        cell.selectButton.selected = NO;
        return ;
    }
...
- (void)setBrowserSelectedItemType:(BOOL)select atIndex:(NSUInteger)index {
    ...
    ////更改Cell 上的选择状态
    TTPhotoGridCell *cell = (TTPhotoGridCell *)[_collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForRow:realSelectIndx inSection:0]];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    TTPhotoGridCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:reuseIdentifier forIndexPath:indexPath];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    TTPhotoTableCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    if (!cell) {
        cell = [[TTPhotoTableCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier];
    }
